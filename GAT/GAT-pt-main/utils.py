...
def load_zebra_finch_data(device='cpu'):
    """
    Loads precomputed features, adjacency matrix, and labels for the Zebra Finch dataset.
    Assumes .npy files are located at ../zebra_finch/
    """
    base_path = os.path.join(os.path.dirname(__file__), '..', 'zebra_finch')
    features = np.load(os.path.join(base_path, 'features.npy'))
    adj = np.load(os.path.join(base_path, 'adjacency_matrix.npy'))
